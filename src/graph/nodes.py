...
def reporter_node(state: State, config: RunnableConfig) -> Command[Literal["__end__"]]:
...
def research_team_node(state: State):
    ...
    for idx, step in enumerate(current_plan.steps):
        if not step.execution_res:
            current_step = step
            logger.debug(f"Found unexecuted step at index {idx}: {step.title}")
            break
    
    if not current_step:
        # All steps completed
        logger.info("All research steps have been completed")
        return Command(
            update=preserve_state_meta_fields(state),
            goto="reporter"
        )
    
    # Determine which agent to use based on step type
    step_type = getattr(current_step, 'step_type', 'research')  # Default to research
    logger.info(f"Routing step '{current_step.title}' (type: {step_type}) to appropriate agent")
    ...
    else:  # Default to researcher for all other types
        return Command(
            update=preserve_state_meta_fields(state),
            goto="researcher"
        )
