      ...
      message.toolCalls.forEach((toolCall) => {
        if (toolCall.argsChunks?.length) {
          try {
            ...
            console.warn("Failed to parse tool call args as JSON:", {
              args: toolCall.argsChunks.join(""),
              error: error instanceof Error ? error.message : String(error)
            });
            // 如果JSON解析失败，将原始字符串包装在一个对象中作为args的值
            toolCall.args = { rawArgs: toolCall.argsChunks.join("") };
          }
